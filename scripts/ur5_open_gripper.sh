...
if [[ "${FORCE_ROS2_CONTROL:-0}" != "1" ]] && gazebo_running; then
  if gz_pub_double "/rg2_finger_joint1_cmd" "0.0" && gz_pub_double "/rg2_finger_joint2_cmd" "0.0"; then
    exit 0
  fi
  PUB_OPTS="--once --wait-matching-subscriptions 0"
  ros2 topic pub $PUB_OPTS "$TOPIC_J1" std_msgs/msg/Float64 "data: 0.0"
  ros2 topic pub $PUB_OPTS "$TOPIC_J2" std_msgs/msg/Float64 "data: 0.0"
  exit 0
fi
...
if echo "$CTRL_OUT" | grep -qE "^gripper_controller[[:space:]].*\\bactive\\b"; then
  PUB_OPTS="--once --wait-matching-subscriptions 0"
  ros2 topic pub $PUB_OPTS "$TOPIC_GRIPPER" std_msgs/msg/Float64MultiArray "{data: [0.0, 0.0]}"
else
  PUB_OPTS="--once --wait-matching-subscriptions 0"
  ros2 topic pub $PUB_OPTS "$TOPIC_J1" std_msgs/msg/Float64 "data: 0.0"
  ros2 topic pub $PUB_OPTS "$TOPIC_J2" std_msgs/msg/Float64 "data: 0.0"
