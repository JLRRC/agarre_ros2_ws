...
echo "[INFO] Lanzando Gazebo **SIN GUI** (modo servidor/headless)..."
echo "[INFO] World: $WS_DIR/$WORLD_FILE"

# -s = solo servidor (sin GUI)
# -r = ejecutar la simulación directamente (no pausada)
# --headless-rendering evita intentar abrir contexto gráfico
gz sim -s -r "$WORLD_FILE" --headless-rendering --verbose
