#!/usr/bin/env bash
# URL: /home/laboratorio/TFM/agarre_ros2_ws/scripts/patch_go_home_v1.sh
# Summary: Writes a robust ur5_go_home.sh script and backs up the old one.
set -Eeuo pipefail

WS_DIR="${WS_DIR:-$HOME/TFM/agarre_ros2_ws}"
DST1="$WS_DIR/scripts/ur5_go_home.sh"
DST2="$WS_DIR/src/ur5_tools/scripts/ur5_go_home.sh"

STAMP="$(date +%Y%m%d_%H%M%S)"
BK_DIR="$WS_DIR/backups/go_home_patch_${STAMP}"
mkdir -p "$BK_DIR"

echo "[INFO] WS_DIR=$WS_DIR"
echo "[INFO] Backup dir: $BK_DIR"

# Backup si existen
[[ -f "$DST1" ]] && cp -a "$DST1" "$BK_DIR/ur5_go_home.sh.scripts.bak" || true
[[ -f "$DST2" ]] && cp -a "$DST2" "$BK_DIR/ur5_go_home.sh.ur5_tools.bak" || true

# Genera el script (contenido único)
TMP="$(mktemp)"
cat > "$TMP" <<'GO'
#!/usr/bin/env bash
...
# Escribe en scripts/
install -Dm755 "$TMP" "$DST1"
echo "[OK] Escrito: $DST1"

# Escribe también en ur5_tools si existe el directorio
if [[ -d "$(dirname "$DST2")" ]]; then
  install -Dm755 "$TMP" "$DST2"
  echo "[OK] Escrito: $DST2"
else
  echo "[INFO] No existe $(dirname "$DST2") (no copio ahí)."
fi

rm -f "$TMP"
echo "[DONE] Patch HOME aplicado."
