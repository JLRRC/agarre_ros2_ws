...
class ControlPanelV2(QMainWindow):
    ...
    retry_send_joints = pyqtSignal()  # Señal para retry desde worker thread

    def _toggle_gripper(self, closed: bool):
        """Compatibilidad: permite controlar el gripper desde la lógica de pick demo."""
        self._toggle_gripper_button(closed)
    ...
    def _build_ui(self) -> None:
        ...
        self.btn_start_all.clicked.connect(lambda: self._run_script("start_all.sh", "START ALL"))
        self.btn_stop_all.clicked.connect(lambda: self._run_script("stop_all.sh", "STOP ALL"))
        self.btn_kill_hard.clicked.connect(lambda: self._run_script("kill_all.sh", "KILL HARD"))
        ...
        self.btn_bridge_start.clicked.connect(self._start_bridge)
        self.btn_bridge_stop.clicked.connect(self._stop_bridge)
        ...
        self.btn_bag_start.clicked.connect(self._start_bag)
        self.btn_bag_stop.clicked.connect(self._stop_bag)
        ...
        # Sistema (CPU/RAM/Load) al lado de los LEDs
        sys_group = QGroupBox("")
        ...
        # Cámara Intel (columna izquierda)
        cam_group = QGroupBox("")
        ...
        # Agregar secciones de información (DOF UR5, Pinza RG2, Cinemática, Dinámica)
        ...
        # Columna izquierda: Cámara + Robot DEMO
        left_col = QVBoxLayout()
        left_col.setSpacing(6)
        left_col.addWidget(cam_group, 2)      # Cámara ocupa más espacio
        left_col.addWidget(g_robot, 1)        # Robot DEMO debajo
        left_col.addStretch(1)

        # Columna central-derecha: Control Manual + Lista de Objetos
        right_col = QVBoxLayout()
        right_col.setSpacing(6)
        right_col.addWidget(g_manual, 2)      # Control Manual ocupa más espacio
        ...
        # Fila superior: Cámara + Robot | Control Manual (alineados)
        top_row = QHBoxLayout()
    ...
    def _log_camera_diagnostics(self, reason: str):
        ...
        ctrl_ok = ros2_control_running()
        clock_ok, clock_reason = clock_status()
        bridge_ok, bridge_reason = bridge_status()
        last_age = "n/a"
        ...
        diag = (
            ...
            f"clock={clock_ok}:{clock_reason} bridge={bridge_ok}:{bridge_reason} "
    ...
    def _refresh_camera_topics(self):
        ...
        def worker():
            cmd = bash_preamble(self.ws_dir) + "ros2 topic list || true"
            out, _err = run_cmd_output(cmd, timeout_sec=2.0)
            topics = [t.strip() for t in out.splitlines() if t.strip()]
            candidates = [t for t in topics if _is_camera_topic(t)]
            if candidates:
                ...
                self._set_status("No se detectaron tópicos de cámara", error=True)
    ...
    def _start_gazebo(self):
        ...
        def worker():
            ...
            try:
                ...
                self._log("[PHYSICS] Gravedad activa (0 0 -9.81 m/s²) - los objetos caerán automáticamente")
                # Verificar que no existan topics de gripper (attach/detach) heredados
                try:
                    time.sleep(1.0)
                    check_cmd = bash_preamble(self.ws_dir) + build_gz_env(self.gz_partition) + "gz topic -l | grep gripper | wc -l"
                    rc, out, err = self.runner.run_cmd(check_cmd, timeout=4)
                    count = int(out.strip()) if out else 0
                    if count > 0:
                        self._log(f"[WARN] Se encontraron {count} topics gripper/* (posible cache). Reinicia Gazebo si persiste.")
                    else:
                        self._log("[CHECK] Sin topics gripper/* - objetos no están unidos al robot")
                except Exception as chk_err:
                    self._log(f"[WARN] No se pudo verificar topics gripper: {chk_err}")
        ...
        threading.Thread(target=worker, daemon=True).start()
        # Programar ajuste automático de joint2 (-20% desde HOME, sentido negativo)
        self._auto_joint2_move_done = False
        QTimer.singleShot(3200, lambda: self._apply_home_joint2_offset(retries=3))
    ...
    def _throw_objects(self):
        ...
        def worker():
            env = build_gz_env(self.gz_partition)
            # Script para hacer drop de los drop_obj_* en Gazebo
            drop_cmd = (
                "for i in {01..10}; do "
                "  gz model -m drop_obj_${i}_* --set-pose \"0 0 0.3\" 2>/dev/null || true ; "
                "done"
            )
            cmd = bash_preamble(self.ws_dir) + env + f"bash -lc '{drop_cmd}'"
            try:
                rc, out, err = self.runner.run_cmd(cmd, timeout=10)
                if rc == 0:
                    self._log("[THROW] ✅ Objetos lanzados")
                    self._set_status("✅ Objetos lanzados")
                else:
                    self._log(f"[THROW] ⚠ Lanzamiento parcial: {err[:100]}")
                    self._set_status("⚠ Lanzamiento completado (parcial)")
            except Exception as e:
                self._log(f"[THROW] Error: {e}")
                self._set_status(f"Error lanzando objetos: {e}", error=True)
            finally:
                pass
    ...
    def _start_debug_poses(self):
        ...
        self._print_pose_snapshot()
        self._stop_debug_poses()
        world_name = read_world_name(self.world_combo.currentText().strip())
        topic = f"/world/{world_name}/pose/info"
        env = build_gz_env(resolve_gz_partition(self.gz_partition))
        parser_script = f"{self.ws_dir}/scripts/parse_gz_poses.py"
        cmd = bash_preamble(self.ws_dir) + env + with_line_buffer(f"gz topic -e -t {shlex.quote(topic)} | python3 {shlex.quote(parser_script)}")
        self._pose_stream_proc = {}
        self.runner.run_stream("DEBUG", cmd, proc_slot=self._pose_stream_proc, key="proc")
        self._log(f"[DEBUG] Iniciado - streaming de poses y joints")
    ...
    def _stop_debug_poses(self):
        ...
        proc = None
        if hasattr(self, "_pose_stream_proc"):
            proc = self._pose_stream_proc.get("proc")
        if not proc:
            self.btn_debug_joints.setStyleSheet("")
            return
        try:
        ...
        try:
            ...
            pass
        self._pose_stream_proc.clear()
        self.btn_debug_joints.setStyleSheet("")
        self._log(f"[DEBUG] Detenido")

    def _print_pose_snapshot(self):
        """Imprime en una línea: TCP, cesta, mesa y objetos en frame world."""
        objs = get_object_positions() if self._gz_running else {}
        obj_parts = []
    ...
    def _release_objects(self):
        ...
        def worker():
            script = os.path.join(self.ws_dir, "scripts", "release_all_objects.sh")
            if not os.path.isfile(script):
                self._log_error(f"No existe script: {script}")
                self._set_status("Script release_all_objects.sh no encontrado", error=True)
                return
            env = build_gz_env(self.gz_partition)
            cmd = bash_preamble(self.ws_dir) + env + f"'{script}'"
            try:
                rc, out, err = self.runner.run_cmd(cmd, timeout=6)
                if out:
                    self._log(out.strip())
                if rc == 0:
                    self._set_status("✅ Objetos soltados")
                else:
                    self._set_status("⚠ Soltar objetos parcial")
            except Exception as e:
    ...
    def _start_bridge(self):
        ...
        gz_ok = self._gz_running or gz_sim_status()
        if not gz_ok:
    ...
    def _refresh_status_sync(self):
        ...
        gz_ok, _ = gz_sim_status()
        br_ok, _ = bridge_status()
        clock_ok, _ = clock_status()
    ...
    def _refresh_controls(self):
        ...
        # Habilitar/deshabilitar controles dependiendo del estado de Gazebo
        # Cámara: habilitada cuando Gazebo está corriendo
        self.camera_topic_combo.setEnabled(self._gz_running)
        self.btn_camera_refresh.setEnabled(self._gz_running)
        self.btn_camera_connect.setEnabled(self._gz_running)
        self.btn_calibrate.setEnabled(self._gz_running)
        
        # Control manual: habilitado cuando Gazebo está corriendo
        manual_enabled = self._gz_running and not self._script_motion_active
    ...
    def _update_ui_state(self):
        ...
        # Cámara (deshabilitada hasta que Gazebo esté corriendo)
        ...
        # Control manual (deshabilitado hasta que Gazebo esté corriendo)
    ...
    def _apply_home_joint2_offset(self, retries: int = 2):
        ...
        env_overrides = " ".join(f"HOME_POS_{i}={target[i]:.6f}" for i in range(6))
        cmd = (
            bash_preamble(self.ws_dir)
            + env_overrides
            + " HOME_TIMEOUT=15 TSEC=3 "
            + f"'{SCRIPTS_DIR}/ur5_go_home.sh'"
        )

        self._log(f"[AUTO] Ajuste joint2=-20% desde HOME -> {target[1]:.3f} rad")
        rc, stdout, stderr = self.runner.run_cmd(cmd, timeout=18)
        if rc == 0:
            self._auto_joint2_move_done = True
            self._set_status("AUTO: joint2 ajustado (-20% HOME)")
        else:
            msg = stderr or stdout
            self._log_warning(f"[AUTO] Falló mover joint2 (-20%): {msg[:120] if msg else 'sin log'}")
            if retries > 0:
                QTimer.singleShot(2000, lambda: self._apply_home_joint2_offset(retries=retries - 1))
    ...
    def _run_pick_demo(self):
        ...
        self._log("[PICK] Secuencia MoveIt-only iniciada")
        ...
        def worker():
            try:
                ...
                self._log("[PICK] Secuencia MoveIt publicada correctamente.")
            except Exception as exc:
                self._set_status(f"Error en pick demo: {exc}", error=True)
                self._log(f"[PICK] ✗ Error: {exc}")
            finally:
                self._set_motion_lock(False)
    ...
    def _log_selection_tf(self, world_pose: Tuple[float, float, float], frame: str = WORLD_FRAME or "world"):
        ...
        if not world_pose:
            return
        tf_status = diagnose_tf_tree(world_pose, selection_frame=frame)
        self._last_tf_status = tf_status
        base_frame_label = tf_status.get("base_frame") or BASE_FRAME or "base"
        world_frame = tf_status.get("world_frame") or frame
        ...
        self._log(
            f"[PICK] TF_DISCOVERY: base={base_frame_label} world={world_frame}"
        )
        tf_lookup_status = "ok" if tf_status.get("ok") else "fail"
        self._log(
            f"[PICK] tf_lookup: {tf_lookup_status} "
            f"world={world_frame} base={base_frame_label} err={tf_status.get('error') or 'n/a'}"
        )
        if tf_status.get("ok"):
        ...
        debug_tf, debug_err = debug_dump_tf(world_frame, base_frame_label)
        if debug_tf:
    ...
    def _resolve_trace_frames(self, world_frame: str) -> Tuple[str, Optional[str]]:
        ...
        effective_base = detected_base or self._base_frame_effective or BASE_FRAME or "base"
    ...
    def _refresh_trace_data(self):
        if not self.trace_table or not self._trace_ready or not self._bridge_ready:
            return
        ...
        if last_world:
            ...
            if last_base and last_base[3] == base_frame:
                ...
                coords, _ = transform_point_to_frame((wx, wy, wz), base_frame, source_frame=source_frame_for_transform)
                if coords:
        ...
        tcp_base_data = None
        if ee_frame:
            tcp_world_data, _ = get_pose(world_frame, ee_frame)
            tcp_base_data, _ = get_pose(base_frame, ee_frame)
        else:
            if now - self._last_ee_warn_ts >= self._ee_warn_period:
                self._log("[TRACE] EE frame unavailable (retrying every 1s)")
        ...
        def worker():
            ...
            try:
                if not robot_control_available():
                    self._set_status("ros2_control/Gazebo no activos", error=True)
                    return
                
                # Verificar si el topic tiene suscriptores
                topic = detect_arm_trajectory_topic()
                check_cmd = (
                    bash_preamble(self.ws_dir)
                    + f"timeout 3 ros2 topic info {shlex.quote(topic)} 2>&1 | grep -E 'Type|Count|Publishers|Subscriptions'"
                )
                rc_check, out_check, _ = self.runner.run_cmd(check_cmd, timeout=4)
                
                if self._debug_logs_enabled:
                    self._log(f"[MANUAL] Topic: {topic}")
                    if rc_check == 0 and out_check:
                        self._log(f"[MANUAL] Info: {out_check.strip()}")
                    else:
                        self._log(f"[MANUAL] ⚠ No se pudo obtener info del topic")
                ...
                if self._debug_logs_enabled:
                    # Mostrar comando abreviado para debug
                    cmd_abbrev = cmd.replace(json.dumps(msg), "{'msg': '...truncated...'}")[:200]
                    self._log(f"[MANUAL] CMD: {cmd_abbrev}...")
                timeout = max(8.0, sec + 2.0)
                rc, stdout, stderr = self.runner.run_cmd(cmd, timeout=timeout)
                if rc == 0:
                    self._set_status(f"Trayectoria enviada a {topic}")
                    if self._debug_logs_enabled:
                        self._log(f"[MANUAL] ✓ Comando enviado (rc=0)")
                else:
                    err = stderr or stdout or "error publicando trayectoria"
                    self._set_status(f"Error moviendo articulaciones: {err[:140]}", error=True)
                    if self._debug_logs_enabled:
                        self._log(f"[MANUAL] ✗ Error rc={rc}: {err[:200]}")
            finally:
                self._manual_inflight = False
